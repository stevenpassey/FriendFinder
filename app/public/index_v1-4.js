...
function changeSlide(slideName)
{
	...
	switch(slideName)
	{
		case 'survey':
			...
			setTimeout(function () { 
				document.getElementById("survey-div").style.transition = "opacity linear 750ms, transform cubic-bezier(.17,.67,.16,.99) 1s";
		...
		case 'slide':
		    ...
		    document.querySelector("#text_overlay button").textContent = SURVEY_COMPLETE === true ? "Survey Results" : "Continue survey";
...
function deleteFrame()
{
	...
	if(document.getElementsByTagName("iframe")[0] && document.getElementById("location-status-bar-text"))
	{
...
function postToFriends()
{
	...
	xhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) 
		{
			...
			document.getElementById("survey-div").innerHTML = matchHTML_string;
			
			matchingNameArray.forEach((friend, index) => { 
				var myBreakdownTable = 
				`<span class='survey-title'>Breakdown - ${matchingNameArray[index].name}</span>
				<div class="Rtable Rtable--3cols">

				  <div style="order:0;" class="Rtable-cell Rtable-cell--head"><h3>${NAME_INPUT.value}</h3></div>
					  ${Array(10).join(0).split(0).map((cell, i) => `
						<div style="order: ${i+1};" class="Rtable-cell">${USER_ANS[i+1]}</div>
					  `).join('')}
				  <div style="order: 11;" class="Rtable-cell Rtable-cell--foot"><strong></strong></div>
				  
				  <div style="order:0;" class="Rtable-cell Rtable-cell--head"><h3>${matchingNameArray[index].name}</h3></div>
					  ${Array(10).join(0).split(0).map((cell, i) => `
						<div style="order: ${i+1};" class="Rtable-cell">${matchingNameArray[index].scores[i]}</div>
					  `).join('')}
				  <div style="order: 11;" class="Rtable-cell Rtable-cell--foot"><strong></strong></div>
				  
				  <div style="order:0;" class="Rtable-cell Rtable-cell--head"><h3>Difference</h3></div>
					  ${Array(10).join(0).split(0).map((cell, i) => `
						<div style="order: ${i+1};" class="Rtable-cell ${cellCalc(addThisArray[i])}">${addThisArray[i]}</div>
					  `).join('')}
